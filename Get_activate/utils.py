...
def get_protgpt2_activations_bau(model, batch, device): 
    ...
    with torch.no_grad():
        ...
        for head in HEADS:
            if torch.isinf(ret[head].output).any():
                print(f"Warning: head_wise_hidden_states has nan!")
        for mlp in MLPS:
            if torch.isinf(ret[mlp].output).any():
                print(f"Warning: mlp_wise_hidden_states has nan!")
        
        # (layers, batch_size, sequence_length, emb_size)
        ...
        # 使用最后一个token的激活(没明白400啥意思，是层数吗)
        final_pos =batch["final_pos"]  # shape: (batch_size,) batch_size=64
        final_pos-=1
        head_wise_hidden_states = head_wise_hidden_states[np.arange(head_wise_hidden_states.shape[0])[:, None], 
                                                            np.arange(head_wise_hidden_states.shape[1]), 
                                                            final_pos]  # shape: (layers, batch_size, 1, emb_size) (36,64,400,1280)
        head_wise_hidden_states = head_wise_hidden_states.transpose(1, 0, 2)  # shape: (batch_size, layers, emb_size)

        mlp_wise_hidden_states = mlp_wise_hidden_states[np.arange(mlp_wise_hidden_states.shape[0])[:, None], 
                                                            np.arange(mlp_wise_hidden_states.shape[1]), 
                                                            final_pos]  # shape: (layers, batch_size, 1, emb_size)
        mlp_wise_hidden_states = mlp_wise_hidden_states.transpose(1, 0, 2)  # shape: (batch_size, layers, emb_size)

        # (batch_size, layers, emb_size)
        # head_wise_hidden_states = head_wise_hidden_states[:, :, -1, :].permute(1, 0, 2)
        # mlp_wise_hidden_states = head_wise_hidden_states[:, :, -1, :].permute(1, 0, 2)
        
    # 其中head_wise_hidden_states是没有针对head进行拆分的。
