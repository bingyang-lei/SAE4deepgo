#!/usr/bin/env python
# 如果使用protgpt2评估，则需要显式设置-m protgpt2 -td test_gpt
import numpy as np
import pandas as pd
import click as ck
...
@ck.option(
    '--test-data-name', '-td', default='test', type=ck.Choice(['test', 'nextprot', 'test_gpt']),
    help='Test data set name')
def main(data_root, ont, model_name, test_data_name):
    if model_name.find('protgpt2')!=-1:
        train_data_file = f'{data_root}/{ont}/train_gpt_data.pkl'
        valid_data_file = f'{data_root}/{ont}/valid_gpt_data.pkl'
    else:
